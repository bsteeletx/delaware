...
void Sprite::init(File FileToInit)
{
	Image::init(FileToInit);
	create();
	...
	while(!FileToInit.FileEOF())
	{
		...
		Line.init(FileToInit.getLine().getString());
		Line.splitAtDelimeter(&Start, &End, ':');

		if (Start.getString() == "Size")
		{
			setSize((float) agk::Val(End.getString()));
			break;
		}
	}
...
void Sprite::parentInit(Text Filename, bool blackIsAlpha, unsigned int assignedSpriteNumber, unsigned int assignedImageNumber)
{
	Image::init(0, Filename, blackIsAlpha, assignedImageNumber);
	create(assignedSpriteNumber);
}

void Sprite::parentInit(const char filename[], bool blackIsAlpha, unsigned int assignedSpriteNumber, unsigned int assignedImageNumber)
{
	Text Filename = Text();
...
void Sprite::subInit(unsigned int parentImage, Text SubImageFilename, unsigned int assignedSpriteNumber, unsigned int assignedImageNumber)
{
	Image::init(parentImage, SubImageFilename, false, assignedImageNumber);
	create(assignedSpriteNumber);
}

void Sprite::subInit(unsigned int parentImage, const char subImageFileName[], unsigned int assignedSpriteNumber, unsigned int assignedImageNumber)
{
...
{
	agk::SetSpritePositionByOffset(spriteNumber, x, y);
}
