...
void Sound::init(Text Filename, unsigned int assignedNumber)
{
	if (assignedNumber)
	{
...
void Sound::init(const char filename[], unsigned int assignedNumber)
{
	Text Filename = Text();
	...
	if (assignedNumber)
	{
		if (_Filename(Filename))
		{
			agk::LoadSound(assignedNumber, filename);
			soundID = assignedNumber;
		}
	}
	else
