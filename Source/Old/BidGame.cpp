...
bool BidGame::updateBid(Theme *CurrentTheme)  //returns true if bid, false if not
{
	float x = 0.0f;
	float y = 0.0f;
	//bool passed = false;
	...
	if (bidder == -1)
	{
		CurrentTheme->showBidBubble((dealer + 1)%4, -1);
		agk::Sync();
	...
	{
	case WEST:
		if (!wPass && (!getBidDecided()))
			CurrentTheme->showBidBubble(WEST, -1);
		...
		if (wPass)
		{
			...
			{
				CurrentTheme->hideBidBubble(WEST);
				//passed = true;
				return false;
			}
		}
		break;
	case NORTH:
		if (!nPass && (!getBidDecided()))
			CurrentTheme->showBidBubble(NORTH, -1);
		...
		if (nPass)
		{
			...
			{
				CurrentTheme->hideBidBubble(NORTH);
				//passed = true;
				return false;
			}
		}
		break;
	case EAST:
		if (!ePass && (!getBidDecided()))
			CurrentTheme->showBidBubble(EAST, -1);
		...
		if (ePass)
		{
			...
			{
				CurrentTheme->hideBidBubble(EAST);
				//passed = true;
				return false;
			}
		}
		break;
	case SOUTH:
		if (!sPass && (!getBidDecided()))
			CurrentTheme->showBidBubble(SOUTH, -1);
		...
		if (sPass)
		{
			...
			{
				CurrentTheme->hideBidBubble(SOUTH);
				//passed = true;
				return false;
			}
		}
	...
	CurrentTheme->showBidBubble(bidder%4, bid);
